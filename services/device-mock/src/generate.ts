...
export function genRecord(equipmentCode: string, userId: string) {
  ...
  return { equipmentCode, uuid: `uuid_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, userId, checkTime, foreheadTemp, normalTemperatureRange: normal, abnormalTemp, handCheckResult, healthAskResult, health };
}
...
export async function postToSchool(schoolBase: string, rec: any) {
  ...
  const faceBuf = Buffer.from('face'); const palmBuf = Buffer.from('palm'); const backBuf = Buffer.from('backOfHand');
...
export async function pushToRegulator(regBase: string, schoolId: string, schoolName: string, payload: any) {
  ...
  const res = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ schoolId, schoolName, ...payload }) });
  return res.json();
}
