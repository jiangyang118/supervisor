  ...
  <el-dialog v-model="createVisible" title="新增入库" width="520px">
    <el-form :model="form" label-width="96px">
      ...
      <el-form-item label="供应商">
        <el-select v-model="form.supplierId" placeholder="请选择" style="width: 240px">
...
function supplierName(id?: string) {
  return suppliers.value.find((s: any) => s.id === id)?.name || '-';
}
...
async function load() {
  rows.value = await api.invInboundList(getCurrentSchoolId());
...
const form = reactive<{ productId: string; qty: number; supplierId?: string }>({
  productId: '',
  qty: 1,
});
const openCreate = () => {
...
async function save() {
  ...
  await api.invInboundCreate({ ...form, schoolId: getCurrentSchoolId() } as any);
