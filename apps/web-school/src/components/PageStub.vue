<template>
  <el-card>
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
      ...
      <div>
        <el-button v-if="actions.includes('create')" type="primary" @click="$emit('create')">新建</el-button>
        <el-button v-if="actions.includes('export-csv')" @click="onExport('csv')">导出 CSV</el-button>
        <el-button v-if="actions.includes('export-pdf')" @click="onExport('pdf')">导出 PDF</el-button>
      </div>
    </div>
    <slot name="filters">
      <el-row :gutter="8" style="margin-bottom:8px;">
        <el-col :span="6"><el-input v-model="q" placeholder="关键词搜索" clearable /></el-col>
        <el-col :span="6"><el-date-picker v-model="range" type="daterange" start-placeholder="开始日期" end-placeholder="结束日期" unlink-panels /></el-col>
...
watchEffect(() => {
  void q.value; void range.value; // 占位，触发依赖
});
</script>
