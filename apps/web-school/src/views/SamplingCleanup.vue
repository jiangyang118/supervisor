...
const rows = ref<Row[]>([
  { id: 'CL-001', sample: '午餐菜品A', method: '销毁', by: '李四', at: new Date().toLocaleString() },
]);
const createVisible = ref(false);
const form = reactive({ sample: '', method: '销毁', by: '' });
const openCreate = () => { createVisible.value = true; };
const save = () => {
  rows.value.unshift({ id: `CL-${String(rows.value.length+1).padStart(3,'0')}`, sample: form.sample, method: form.method, by: form.by, at: new Date().toLocaleString() });
  createVisible.value = false;
};
const onExportCsv = () => exportCsv('样品清理', rows.value, { id:'ID', sample:'样品', method:'处理方式', by:'处理人', at:'时间' });
</script>
